<template>
    ...
    >
      ...
      <p class="title">审核新闻</p>
      <div class="outerItem" v-for="(item, i) in newsList" :key="i" >
        <div class="actions" >
          <div>
        ...
        <div class="newsItem" @click="toEdit(item)">
          <p class="newsTitle">
            <span>{{
              item.status === 1 ? "【置顶】" : item.status === 2 ? "【隐藏，待审核】" : ""
            }}</span>
            {{ item.title }}
          </p>
        ...
        getNews().then(({ code: code, result: res, message: msg }) => {
          if (code === 0) {
            ...
            for (let i = 0; i < this.total; i++) {
                if(res[i].status == 2)
                    {
                        console.log(new Date(res[i].releaseTime).getTime()+60480)
                        console.log(new Date().getTime())
                        if(new Date(res[i].releaseTime).getTime()+604800000*4 > new Date().getTime())
